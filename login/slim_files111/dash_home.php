...
$app->post('/playing11',function() use ($app){
 ...
 $app->response()->header("Content-Type", "application/json");
    ...
    for($i=0;$i<11;$i++){
     //console.log($data);
     $query="INSERT INTO personal_team_info values(2,'$data[$i]')";
      $result=mysql_db_query('minor_project',$query,$linkid);
    }
...
$app->post('/sub',function() use ($app){
 ...
 $app->response()->header("Content-Type", "application/json");
    ...
    for($i=0;$i<4;$i++){
     //console.log($data);
     $query="INSERT INTO substitutes values(2,'$data[$i]')";
      $result=mysql_db_query('minor_project',$query,$linkid);
    }
...
$app->post('/formation',function() use ($app){
 ...
 $app->response()->header("Content-Type", "application/json");
   ...
   //  $query="INSERT INTO formation values(2,'$data[$i]','$data[$j],'$data[$k]')";
    ...
    $query="INSERT INTO formation  values (2,'$data[0]','$data[1]','$data[2]')";
      $result=mysql_db_query('minor_project',$query,$linkid);
